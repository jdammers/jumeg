...
class JuMEGVertexBuffer(object):
      ...
      def __init__(self, usage=GL_DYNAMIC_DRAW):
          ...
          self.attribs = [b"vertex", b"tex_coord", b"normal", b"color"]
          self.locations = dict((k, v) for (v, k) in enumerate(self.attribs))
          self.uniforms = [b"lighting", b"texturing", b"texture_3d"]
      ...
      def vbo_init(self):
          #self._vbo_id = GLuint()
          #glGenBuffers(1, pointer(self._vbo_id))
          ...
          # glBufferData(GL_ARRAY_BUFFER, self.data_buffer_size, 0, GL_DYNAMIC_DRAW)


      def vbo_update(self):
          glBindBuffer(GL_ARRAY_BUFFER, self._vbo_id)
          #glBufferData(GL_ARRAY_BUFFER, sizeof(self.data), 0, GL_DYNAMIC_DRAW)
          glBufferSubData(GL_ARRAY_BUFFER, 0,self.data)
          print"UPDATE OK"

      def vbo_draw(self):
          glBindBuffer(GL_ARRAY_BUFFER, self._vbo_id)
          glEnableVertexAttribArray( self.locations[b"vertex"] );
          glVertexAttribPointer( self.locations[b"vertex"],2,GL_FLOAT,GL_FALSE,0,0)
          ...
          # glEnableClientState(GL_VERTEX_ARRAY)
          #glVertexPointer(2, GL_FLOAT,2*sizeof(c_float),self._vbo_id )
          #glDrawArrays(GL_LINE_STRIP, 0, self.data_points-1)
    ...
    #---
      ...
      def __get_data_buffer_size(self):
         return  sizeof(c_float) * self.data.size

      data_buffer_size=property(__get_data_buffer_size)
"""




class JuMEGVertexBufferV1(object):
