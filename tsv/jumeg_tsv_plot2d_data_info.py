...
class PLT_COLOR( object ):
      ...
      def update(self):          
          self.label_list = self.__wxcdb.getColourList()
          self.info_list  = self.__wxcdb.getColourInfoList()
      ...
      def index2color(self,index):
          return self.info_list[index][1:]    
    
      def label2color(self,label):
          return np.array( self.info_list[ self.label_list.index( label.upper() ) ],dtype=np.float32)   
...
# grous=[]
# for g in group_list
#     group.append(INFO_GROUP(label=g))
#      oder
# group[g]=INFO_GROUP(label=g))
#

#----------------------------------------
class PLT_CHANNELS(object):
...
#--- TODO implement chanel info dict list
# label index 
#     label_list = raw.info['channels'] 
#     index_list

#     info={label:{ index:0,scaling:{ range:{min=-1.0,max:1.0},unit:'pT',type:'adjust'} },color:(r,g,b,a) }
   
     
#class INFO_GLOBAL( object):
#      """
#         Helper class for info options
#      """
#
#      def __init__ (self):           
#          self.channels = INFO_CHANNELS()          
#          self.groups   = INFO_GROUPS()
...
class JuMEG_TSV_PLOT2D_DATA_INFO(object):
      def __init__ (self,raw=None) :
          ...
          self.plt_channels = PLT_CHANNELS()
          
          self.group      = {}
          self.group_list = []
          self.channels   = {}
          self.group_label_list = ['grad','mag','eeg','stim','eog','emg','ecg','ref_meg','resp','exci','ias','syst','misc','seeg','chpi']
          self.group_color_list = ['BLACK','RED','GREEN','MAGENTA','BLUE','BROWN','CYAN','DARKGREEN','DARKORANGE','TURQUOISE','VIOLET','GREY50','GREY50','GREY50','GREY50']
    ...
    #-- TDOD exclude UTl channel    
      ...
      def init_info(self,raw=None):
          ...
          if self.raw: 
             self.plt_channels.init(self.raw.info['nchan'])
      ...
      def init_group_info(self):
          ...
          for idx in range( self.raw.info['nchan'] ):
             #--- get group for idx meg eeg ... 
              ...
              self.group[g].color_index = self.plt_color.label2index(self.group[g].color)
           
           #--- working with numpy arrays in pl_channels class
              self.plt_channels.color_index[idx] = self.group[g].color_index
              self.plt_channels.selected[idx]    = True
              self.plt_channels.label.append( self.raw.info['ch_names'][idx] )
              self.plt_channels.scale[idx]       = 1.0
              self.plt_channels.group_index[idx] = self.group[g].group_index
                    
          for g in self.group_list:
              self.group[g].index = np.array( self.group[g].index_list,dtype=np.float32 )

          if self.raw.info['bads']:          
             self.plt_channels.bads[ self.raw.info['bads'] ] = False
