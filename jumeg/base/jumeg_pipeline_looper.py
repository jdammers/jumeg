...
class JuMEG_PipelineLooper(JuMEG_PDF_BASE):
    ...
    def init(self,options=None,defaults=None):
       ...
       #--- logfile
        ...
        self._PDFList.list_file_path = self.get_value("list_path",[opt,defaults,cfg_global])
        self._PDFList.list_file_name = self.get_value("list_name",[opt,defaults,cfg_global])
        self._PDFFile.path           = self.get_value("fpath",    [opt,defaults,cfg_global])
        self._PDFFile.name           = self.get_value("fname",    [opt,defaults,cfg_global])
        
        
    def init_logfile(self):
        self._Hlog = jumeg_logger.update_filehandler(logger=logger,path="./log",prefix=self.logprefix,name=os.path.splitext(self.pdf.name)[0])
...
def test4():
    ...
    for raw_fname,subject_id,dir in jpl.file_list():
        print("Test loop : {}".format(jpl.recursive))
