    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"e:s:p:f:c,rvhd",
                             ["exp=","experiment=","condi=","condition=","stage=","pfif=","fif=","ffif=",
                              "plist=","path_list=","flist=","bads=","help","run","verbose"])
...
#--- init experiment template parameter
    ...
    if fname_list:
       fn_raw_list,fn_raw_bad_channel_dict = jumeg_base.get_filename_list_from_file(path_list + "/" + fname_list,start_path = path_mne_stage)
   ...
   #--- check & add fif file to list update bad channel dict
    if fif_name:
       ...
       if os.path.isfile(f):
          ...
          if bad_channels:
             fn_raw_bad_channel_dict[f]= bad_channels
