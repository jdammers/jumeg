...
#--- jumegs functions
import jumeg.preprocbatch.jumeg_preprocbatch        as jppd
import jumeg.preprocbatch.jumeg_preprocbatch_utils  as jpp_utils
  ...
  #--- filter after ocarta    
        if hasattr(tmp_pp_raw.filter_after_ocarta,"do_run"):
           if tmp_pp_raw.filter_after_ocarta.do_run :
              tmp_pp_raw.filter_after_ocarta['verbose'] = opt.verbose
              print"\n===> PP Info: start apply filter after ocarta:"
              print"File  : " + fif_file
              if opt.verbose:
                 print"Parameter :"
                 print tmp_pp_raw.filter_after_ocarta
                 print"\n\n"
              (fif_file, raw) = jppd.apply_filter_data(fif_file,raw=raw,**tmp_pp_raw.filter_after_ocarta)
              print"\n\n==> PP Info: done apply filter after ocarta\n  ---> " + fname
           else:
              fif_file = jumeg_base.get_fif_name(fname=fif_file,postfix=tmp_pp_raw.filter_after_ocarta.fif_postfix,extention=tmp_pp_raw.filter_after_ocarta.fif_extention) 
              raw   = None
        else:
